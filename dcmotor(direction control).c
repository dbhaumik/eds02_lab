...
void PortF_Init(void){ unsigned long volatile delay;
  ...
  GPIO_PORTF_AMSEL_R &= ~0x23;      // no analog 
  GPIO_PORTF_PCTL_R &= ~0x00F000FF; // regular function
  GPIO_PORTF_DIR_R &= 0x07;     // make PA5,PA0,PA1 out
  GPIO_PORTF_DR8R_R |= 0x20;    // can drive up to 8mA out
  GPIO_PORTF_AFSEL_R &= ~0x23;  // disable alt funct on PA5,PA1,PA0
  GPIO_PORTF_DEN_R |= 0x23;     // enable digital I/O on PA5,PA1,PA0
}
...
void PortA_Init(void){ unsigned long volatile delay;
  ...
  GPIO_PORTA_PCTL_R &= ~0x00F000FF; // regular function
  ...
  GPIO_PORTA_DEN_R |= 0x23;     // enable digital I/O on PA5,PA1,PA0
}
